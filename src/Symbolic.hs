...
data SymExtension f a where
  SymExtension :: SymContainer f
    => SymShape f -> (SymPosition f -> SBV a) -> SymExtension f a

data SymMorphism f g where
  SymMorphism :: (SymContainer f, SymContainer g)
    => (SymShape f -> SymShape g)
    -> (SymShape f -> SymPosition g -> SymPosition f)
    -> SymMorphism f g
...
io :: forall f g a. SymMorphism f g -> SymExtension f a -> SymExtension g a
  -> Symbolic ()
io (SymMorphism u g) (SymExtension s p) (SymExtension t q) = do
  ...
  constrain \(Forall x) -> depend @(Shape g) @(Position g) undefined t x .=>
    let y = g s x in
      depend @(Shape f) @(Position f) undefined s y .&& p y .== q x
...
symbolicContainer :: (SymContainer f, HasKind a) => String -> String
  -> Symbolic (SymExtension f a)
...
-- NOTE: this does not constrain the shapes and positions based on their dependencies.
symbolicMorphism :: (SymContainer f, SymContainer g) => String -> String
  -> Symbolic (SymMorphism f g)
...
combine :: SymVal a => SymExtension f a -> SymExtension g a ->
  SymExtension (Product f g) a
combine (SymExtension s p) (SymExtension t q) =
  SymExtension (SBV.tuple (s, t)) (SBV.either p q)

-- * Combinators

makeFoldr :: (SymContainer f, SymContainer g, SymContainer h, SymVal a)
  => h a -> [f a] -> g a -> g a -> SymMorphism (Product h (Product f g)) g
  -> String -> Symbolic ()
...
makeMap :: (SymContainer f, SymContainer g, SymContainer h, SymVal a)
  => h a -> [f a] -> [g a] -> SymMorphism (Product h f) g
  -> String -> Symbolic ()
